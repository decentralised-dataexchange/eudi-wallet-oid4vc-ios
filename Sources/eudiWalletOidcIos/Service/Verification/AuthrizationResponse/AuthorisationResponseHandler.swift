...
class AuthorisationResponseHandler {
    ...
    func prepareAuthorisationResponse(credentialsList: [[String]]?,
                                      ...
                                      did: String, keyHandler: SecureKeyProtocol, isSca: Bool, keyIds: [[String]], amrInherenceFactor: String?, amrKnowledgeFactor: String?) async -> [String: Any]?{
        guard let responseMode = ResponseMode(from: presentationRequest?.responseMode ?? "") else { return nil}
        switch responseMode {
        case .directPost:
            var params = await AuthorisationResponseBuilder.buildResponse(credentialsList: credentialsList, presentationRequest: presentationRequest, did: did, keyHandler: keyHandler, isSca: isSca, keyIds: keyIds, amrInherenceFactor: amrInherenceFactor, amrKnowledgeFactor: amrKnowledgeFactor)
        ...
        case .iarPostJWT:
            let params = await AuthorisationResponseBuilder.buildResponse(credentialsList: credentialsList, presentationRequest: presentationRequest, did: did, keyHandler: keyHandler, isSca: isSca, keyIds: keyIds, isEncrypted: true, amrInherenceFactor: amrInherenceFactor, amrKnowledgeFactor: amrKnowledgeFactor)
            do {
